...
cd css
./srcds_run -game cstrike -insecure +exec server.cfg +hostname "$CSS_HOSTNAME" +sv_password "$CSS_PASSWORD" +rcon_password "$RCON_PASSWORD" +map "$CSS_STARTMAP" +sv_lan "$CSS_SVLAN" +maxplayers "$CSS_MAXPLAYERS" -port "$CSS_PORT" -tv_port "$CSS_TV_PORT" -clientport "$CSS_CLIENT_PORT" -tickrate "$CSS_TICKRATE"
